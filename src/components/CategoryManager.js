    ...
    >
      ...
      >
        <div style={{ padding: '22px 22px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
          <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '20px', fontWeight: 800, color: '#0f172a' }}>{t.manageCategories}</h2>
        ...
        <div style={{ padding: '0 22px 22px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#aaa', marginBottom: '8px' }}>{t.defaultLabel}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {defaultCategories.map(cat => (
                <div key={cat.key} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', background: '#f8fafc', borderRadius: '10px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#cbd5e1', fontStyle: 'italic' }}>{t.defaultTag}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {customCategories.length > 0 && (
            <div>
              <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#aaa', marginBottom: '8px' }}>{t.customLabel}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ border: '1.5px dashed #e2e8f0', borderRadius: '14px', padding: '16px' }}>
            <p style={{ fontSize: '13px', fontWeight: 700, color: '#475569', marginBottom: '14px' }}>{t.newCategory}</p>

            <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, marginBottom: '6px' }}>{t.emojiLabel}</p>
            ...
            <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, marginBottom: '6px' }}>{t.colorLabel}</p>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '12px' }}>
              ...
              <input type="color" value={color} onChange={e => setColor(e.target.value)} title={t.customColor} style={{ width: '28px', height: '28px', borderRadius: '50%', border: '2px solid #e2e8f0', cursor: 'pointer', padding: '0', background: 'none' }} />
            </div>

            <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, marginBottom: '6px' }}>{t.nameLabel}</p>
